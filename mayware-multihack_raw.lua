/*
	[Header]
*/
local build_info = "2026-07-11 @ 12:33 UTC"
...
local function Triggerbot(cmd)
	...
	local trace = util.TraceLine({mask = MASK_SHOT, start = eyePos, endpos = eyePos + cmd:GetViewAngles():Forward() * 32768, filter = localplayer})
...
hook.Add("PostDrawOpaqueRenderables", GenerateID(), function()
	cam.Start3D()
	Wallhack() //TODO: fix screen grab
	cam.End3D()
end)
...
print("[Mayware MultiHack] menu key: " .. input.LookupBinding("+menu_context", true) .. ", aim/trigger key: " .. input.GetKeyName(aimtrig_key))
